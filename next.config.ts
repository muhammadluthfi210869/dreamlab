...
import { MAKLON_PRODUCT_REDIRECTS } from './src/data/maklon-redirects';
...
// Duplicate flat-vs-sub-category product routes discovered in products-v2.
// A product slug that exists BOTH at /produk/{cat}/{slug}/ (flat) AND at
// /produk/{cat}/{sub}/{slug}/ (nested under a sub-category) renders the same
// product from two indexable URLs with self-canonicals — Google sees a
// duplicate and may rank the wrong one. Consolidate the nested URL to the
// canonical flat URL with 301.
//
// Sub-category HUB pages (e.g. /produk/decorative/make-up/) are NOT included
// — they remain 200 + indexable and should appear in the sitemap.
const PRODUCT_DUPLICATE_ROUTES: Array<[string, string]> = [
  // DECORATIVE — nested under make-up → flat
  ['/produk/decorative/make-up/liquid-highlighter/', '/produk/decorative/liquid-highlighter/'],
  ['/produk/decorative/make-up/mascara/',         '/produk/decorative/mascara/'],
  ['/produk/decorative/make-up/cream-blush/',     '/produk/decorative/cream-blush/'],
  // DECORATIVE — nested under lipcare → flat
  ['/produk/decorative/lipcare/lip-matte/',       '/produk/decorative/lip-matte/'],
  ['/produk/decorative/lipcare/lip-gloss/',       '/produk/decorative/lip-gloss/'],
  // SKINCARE — nested under face-cream → flat
  ['/produk/skincare/face-cream/moisturizing-cream/', '/produk/skincare/moisturizing-cream/'],
  ['/produk/skincare/face-cream/brightening-cream/',  '/produk/skincare/brightening-cream/'],
  ['/produk/skincare/face-cream/eye-cream/',          '/produk/skincare/eye-cream/'],
  // SKINCARE — nested under cleansing → flat
  ['/produk/skincare/cleansing/micellar-cleansing-gel/', '/produk/skincare/micellar-cleansing-gel/'],
];
    ...
    return [
      {
        ...
        headers: [
          ...
          { key: 'Permissions-Policy', value: 'camera=(), microphone=(), geolocation=()' },
          {
            ...
            value: [
              "default-src 'self'",
              "script-src 'self' 'unsafe-inline' 'unsafe-eval' https://www.googletagmanager.com https://va.tawk.to https://embed.tawk.to https://analytics.tiktok.com https://connect.facebook.net https://*.clarity.ms https://googleads.g.doubleclick.net https://www.googleadservices.com https://forms.kommo.com",
              "style-src 'self' 'unsafe-inline' https://fonts.googleapis.com",
              "font-src 'self' https://fonts.gstatic.com",
              "img-src 'self' data: https: blob:",
              "connect-src 'self' https://*.supabase.co https://dreamlab.id https://va.tawk.to wss://*.tawk.to https://analytics.google.com https://www.google.com https://www.googleadservices.com https://googleads.g.doubleclick.net https://ad.doubleclick.net https://stats.g.doubleclick.net https://analytics.tiktok.com https://connect.facebook.net https://www.facebook.com https://*.clarity.ms https://www.google-analytics.com https://www.googletagmanager.com http://localhost:3002 https://nexerp.id https://api.nexerp.id https://forms.kommo.com https://*.on.aws https://*.run.app",
  ...
  async redirects() {
    ...
    */
    ...
    const legacyRedirects: Array<[string, string]> = [
      ...
      ['/alur-maklon', '/about-us/alur-maklon/'],
      ['/kontak', '/contact-us/'],
      ['/layanan-formulasi', '/services/'],
    ...
    // NOTE: /maklon-skincare|parfum|hair-care are NOT here — they must
    // route to /produk/{category}/ (organic SEO). MAKLON_PRODUCT_REDIRECTS
    // (step 8) owns those sources. /google-ads/{slug} is still served
    // directly as a real route; only the legacy aliases were removed.
    ...
    const wordpressRedirects: Array<[string, string]> = [
      ...
      // Dikembalikan ke target semula agar URL persis seperti sebelum commit SEO
    ...
    // 7. Duplicate flat-vs-sub-category product routes (see PRODUCT_DUPLICATE_ROUTES)
    // Setiap duplikat harus 301 ke versi flat kanonik agar Google mengonsolidasikan
    // sinyal ke satu URL.
    for (const [source, destination] of PRODUCT_DUPLICATE_ROUTES) {
      addRedirect(source, destination);
    }

    // 8. Legacy /maklon-{category}/{product}/ product URLs → /produk/{category}/{product}/
    // Daftar dipusatkan di src/data/maklon-redirects.ts (GENERATED by scripts/generate-maklon-redirects.mjs).
    // Hanya entri yang destination-nya adalah halaman /produk/{category}/{product}/ yang valid yang dipakai.
    for (const [source, destination] of Object.entries(MAKLON_PRODUCT_REDIRECTS)) {
      addRedirect(source.endsWith('/') ? source : `${source}/`, destination);
    }
