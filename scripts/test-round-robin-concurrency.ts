...
async function main() {
  ...
  console.log('  counter akhir: ' + after + ' (20 mod 6 = 2)');
  console.log('  CS unik: ' + Object.keys(counts).length + ' dari 6');
  console.log('  distribusi: ' + JSON.stringify(counts));
  const ok = after === 2 && Object.keys(counts).length === 6 && Object.values(counts).every((c) => c >= 3 && c <= 4);
  console.log(ok ? '  [PASS] 20 paralel visitor berbeda: counter +20, 6 CS rata' : '  [FAIL] ' + JSON.stringify({ after, counts }));
