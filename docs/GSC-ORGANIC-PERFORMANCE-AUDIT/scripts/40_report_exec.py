"""Report generator: executive-summary.md + full-analysis.md"""
...
# ---- Monthly trend table ----
mt = pd.read_csv(os.path.join(OUT, "monthly-trend.csv"))
mt_tbl = md_table(mt.to_dict("records"), ["ym","clicks","impressions","ctr","avg_position"], ["Bulan","Clicks","Impressions","CTR %","Avg Pos"])

# ---- Directory bucket table (proper bucketing) ----
dir_df = pd.read_csv(os.path.join(OUT, "directory-performance.csv"))
dir_df["growth_pct"] = dir_df["growth_pct"].where(pd.notna(dir_df["growth_pct"]), "")
dir_df["growth_pct"] = dir_df["growth_pct"].map(lambda v: f"{v:.0f}%" if isinstance(v, float) else v)
dir_tbl = md_table(dir_df.to_dict("records"), ["bucket","clicks_c90","impressions_c90","clicks_delta","ctr_c90","growth_pct"], ["Direktori","Klik","Impressions","Δ","CTR%","Growth"])

# ---- Executive summary ----
ex = f"""# EXECUTIVE SUMMARY — Organic Performance Intelligence Audit
...
{md_table(ins["page_biggest_losers"][:10], ["url","current_clicks","clicks_delta","current_ctr_pct","current_position","page_type"], ["Halaman","Klik (90d)","Δ Klik","CTR %","Pos","Tipe"])}

**Pendapatan baru (gains 90d):**
{md_table(ins["page_biggest_winners"][:10], ["url","current_clicks","clicks_delta","current_ctr_pct","current_position","page_type"], ["Halaman","Klik (90d)","Δ Klik","CTR %","Pos","Tipe"])}
...
{md_table(ins["page_top_opportunities"][:10], ["url","c90_impressions","current_clicks","current_ctr_pct","current_position","potential_opportunity","page_type"], ["Halaman","Impressions","Klik","CTR%","Pos","Potensi Klik","Tipe"])}

### 5.2 Kata kunci komersial dengan potensi konversi (position 4–15)
{md_table(ins["commercial_queries"][:10], ["query","impressions_c90","clicks_c90","position_c90","ctr_c90_pct"], ["Query","Impressions","Klik","Pos","CTR%"])}
...
with open(os.path.join(OUT, "..", "executive-summary.md"), "w", encoding="utf-8") as f:
    f.write(ex)
print("executive-summary.md written", len(ex), "chars")
