...
import { isRedirectOnlyPath, toCanonicalUrl } from './seo-url-policy';
...
export async function getSEOData(slug: string) {
  ...
  if (isRedirectOnlyPath(slugWithSlash)) {
    return null;
  }
  ...
  if (process.env.NEXT_PUBLIC_SUPABASE_URL && process.env.NEXT_PUBLIC_SUPABASE_URL !== 'https://your-project.supabase.co') {
    ...
    if (data && !error) {
      const canonical = data.canonical_url && !isRedirectOnlyPath(data.canonical_url)
        ? data.canonical_url
        : toCanonicalUrl(slugWithSlash);
      return {
        ...data,
        canonical,
  ...
  return {
    ...
    h1: data.h1,
    canonical: data.canonical && !isRedirectOnlyPath(data.canonical)
      ? data.canonical
      : toCanonicalUrl(slugWithSlash),
    ...
    schema_markup: {
      ...
      "name": data.meta_title,
      "description": data.meta_description,
      "url": data.canonical && !isRedirectOnlyPath(data.canonical)
        ? data.canonical
        : toCanonicalUrl(slugWithSlash)
    },
    faq_json: derivedFAQ
