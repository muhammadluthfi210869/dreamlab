...
const LINKTREE_MSG = "Hi Dreamlab, Saya mengetaui dari media social ingin konsultasi Produk lebih lanjut";
...
export default function LinktreePage() {
  ...
  const processWA = useCallback(() => {
    ...
    setTimeout(() => {
      ...
      const idx = parseInt(saved || "0", 10) % RR_NUMBERS.length;
      const phone = RR_NUMBERS[idx];
      const url = `https://wa.me/${phone}?text=${encodeURIComponent(LINKTREE_MSG)}`;
      ...
      <div className="w-full max-w-[440px] mx-auto flex flex-col items-center">
        ...
        >
          <button
            ...
            id="wa-button"
            disabled={loading}
            className="btn-wa w-full bg-[#24C75D] hover:bg-[#1EAE4F] text-white py-4 px-6 rounded-[22px] flex items-center justify-center gap-4 transition-all duration-300 hover:scale-[1.015] active:scale-98 shadow-[0_8px_25px_rgba(36,199,93,0.22)] hover:shadow-[0_12px_30px_rgba(36,199,93,0.3)] group disabled:opacity-70 disabled:cursor-not-allowed"
          >
            <svg viewBox="0 0 24 24" className={`w-6 h-6 fill-white transition-transform duration-300 group-hover:scale-105 ${loading ? 'animate-spin' : ''}`}>
            ...
            <div className="flex flex-col text-left">
              <span className="text-[12px] font-black uppercase tracking-wider font-onest">{loading ? 'Menghubungkan ke Admin...' : 'Hubungi Kami via WhatsApp'}</span>
              <span className="text-[11px] text-white/90 font-medium font-sans">{loading ? 'Mohon tunggu sebentar' : 'Konsultasi Gratis · Respon Cepat'}</span>
            </div>
          </button>
        </motion.div>
