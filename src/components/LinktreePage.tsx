...
export default function LinktreePage() {
  ...
  const [btnText, setBtnText] = useState("Hubungi Kami via WhatsApp");

  const processWA = useCallback(() => {
    if (loading) return;
    setLoading(true);
    setBtnText("Menghubungkan ke Admin...");
    ...
    setTimeout(() => {
      ...
      setLoading(false);
      setBtnText("Hubungi Kami via WhatsApp");
      ...
      <div className="w-full max-w-[440px] mx-auto flex flex-col items-center">
        ...
        >
          <button
            ...
            disabled={loading}
            id="wa-button"
            className={`btn-wa w-full bg-[#24C75D] hover:bg-[#1EAE4F] text-white py-4 px-6 rounded-[22px] flex items-center justify-center gap-4 transition-all duration-300 hover:scale-[1.015] active:scale-98 shadow-[0_8px_25px_rgba(36,199,93,0.22)] hover:shadow-[0_12px_30px_rgba(36,199,93,0.3)] group ${loading ? "opacity-70" : ""}`}
          >
            {/* WhatsApp SVG Icon */}
            <svg
              viewBox="0 0 24 24"
            ...
            <div className="flex flex-col text-left">
              <span className="text-[12px] font-black uppercase tracking-wider font-onest" id="btn-text">
                {btnText}
              </span>
