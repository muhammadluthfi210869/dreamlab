...
// ─── Product card images (clean product shots from ProductGrid) ───
const productCardImageMap: Record<string, Record<string, string>> = {
  skincare: {
    // Main products
...
export const getProductCardImagePath = (slug: string, categorySlug: string): string | null => {
  const categoryMap = productCardImageMap[categorySlug];
  if (categoryMap && categoryMap[slug]) {
    const path = categoryMap[slug];
    return `/${path.split('/').map(p => p.replace(/ /g, '%20')).join('/')}`;
  }
  return null;
};
